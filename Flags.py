...
You have to choose how many flags you should take with you. The goal is to set the maximum number of flags on the peaks,
...
def solution(A):
    ...
    max_flags = 1 # set the first flag on the first peak
    flags = 2 # start checking from two flags

    while( flags <= len(peak) ):
        total_flags = flags - 1 # minus the first flag on first peak
        previous = 0
        for current in range( 1, len(peak) ):
            if( peak[current] - peak[previous] >= flags ):
                previous = current
                total_flags -= 1
                if( total_flags == 0 ):
                    max_flags = flags
                    break
        if( total_flags > 0 ): # It has achieved the maximum so there are remaining flags
            return max_flags
        flags += 1
    return len(peak)
